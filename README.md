自己开发使用的cocos2d-x库，定期和官方发布最新稳定版本保持一致，稍许修改一些地方，以更加符合自己的喜好
-----------------------

* cocos2d-x版本：cocos2d-x-3.2  Jul.17 2014
* 官方下载站：http://cn.cocos2d-x.org
* 官方源码库：https://github.com/cocos2d/cocos2d-x

1. 修改了忽略文件列表，原有的库忽略文件列表里有编译好的第三方库及对应的头文件，如果没有加入版本控制系统则编译链接失败
2. 对于cpp_tests和cpp_empty_test两个测试工程，提供了windows下NDK一键编译的批处理文件build.bat，用来代替臃肿的py脚本，其中Application.mk中添加了一句APP_PLATFORM := android-14
3. ccUtils.h中line77中的句号和line78中的左括号为中文字符，导致vs2012编码警告，已修改为英文字符
4. .gitmodules文件删除
5. cpp_tests和cpp_empty_test添加user文件来保存工作目录，否则资源载入出错
6. CCBundleReader.h line90: long int tell() ==> ssize_t tell()
7. cocos/2d/cclabel.h line276 返回值强制(int)转换，消除警告
